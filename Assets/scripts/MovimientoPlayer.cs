...
public class MovimientoPlayer : MonoBehaviour
{
    ...
    // Los Update se llaman siempre en cada frame del juego. En este caso llamo a la funcion Reconocer teclas y la ejecutar� en cada fotograma del juego 
    void Update()
    {
        ReconocerTeclas();
    ...
    /*El fixed Update lo utilizar� para calcular las fisicas y movimientos. Se llama fixed Update porque nos soluciona un error muy basico.
    Con un ejemplo se entender� mejor:

    Si tu estas jugando a un juego a 30 fps y otro a 60 fps el Update normal se va a ejecutar mas veces cuantos
    mas frame halla. A la hora de detectar los Input(pulsar teclas) y otras acciones no pasar�a nada, es m�s, el update normal se usa siempre.

    El problema llega cuando tienes que a�adir fuerzas o velocidades (cualquier cosa que tenga fisicas).Si calculase la funcion Mover() en un Update normal
    la velocidad del personaje cambiar�a dependiendo de los fps(en mi ordenador lo veria moviendose a una velocidad de 12 por ejemplo y en otro se ver�a de 20).
    ...
    /*
     ...
     que son inputs que vienen predeterminados y hacen mas facil la tarea. As� no tenemos que decirle las teclas que queremos, solo con Horizontal
